...
import { DesTxtInterFormData, TransferToSameSchemaTwo } from "../utils";

const InternationalTransferTwo = () => {
  const [loading, setLoading] = useState(false);
  const axiosPrivate = useAxiosPrivate();
  const navigate = useNavigate();
  const user = useAppSelector((state) => state.user.user!);
  const { formData } = useContext(TransferToInterContext)!;
  ...
  const sendRequest = async (val: DesTxtInterFormData, sec: typeof values) => {
    if (user.security_pin === sec.security_code) {
      setLoading(true);
      try {
        const { data } = await axiosPrivate.post("/transfer-inter", val);
...
export default InternationalTransferTwo;
